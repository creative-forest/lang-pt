...
//! #### Token Types  
//!
//! First, we will be creating a token type that will represent value of the each element of the token stream data.
//! The token type should implement [TokenImpl] trait so that [Tokenizer] can access the required default End of File(EOF) token.
//! The default EOF token [eof](TokenImpl::eof()) will be added at the end of the token stream,
//!
//! We will define enum types containing various parts of javascript expression and implement [TokenImpl] trait.
//!
//! ```
...
//! The token stream will then feed into the parser to parse the input document into a meaningful Abstract Syntax Tree ([AST](crate::ASTNode)).
//! This library can be used to create a [LexerlessParser] program which discarded the need to separately assign a [Tokenizer].
//! However, a tokenizer often makes a parser perform faster than parsing without a lexer.
//! In this example, we will be creating a [Parser](DefaultParser) program that gets tokenized input from the tokenizer.
//!
//! Before building a complete tokenizer for javascript expression,
//! we will first implement a tokenizer for simple arithmetic expressions consisting of number, identifier, and arithmetic operators.
//! A tokenizer consists of lexeme utilities which are responsible to create tokens at incremental position of the input.
//! Check out the [lexeme](crate::lexeme) module documentation to get overview of the available lexeme utilities and their functionalities.
//!
//! ```
...
//!
//! ```
//! Now we are going to map the tokenized ID into respective keywords.
//! Therefore, we are going to wrap the identifier pattern with [Mapper](crate::lexeme::Mapper) so that
//! it maps keywords and values with associated tokens.   
//! ```no_run
//! let identifier: Pattern<Token> = Pattern::new(Token::ID, r#"^[_$a-zA-Z][_$\w]*"#).unwrap();
...
//! #### Regex literal
//!
//! A regex literal for the Javascript language is defined by /pattern/[g][m][i].
//! Following lexeme utility can be implemented to parse Javascript regex literal.
//!
//! ```
...
//!     .unwrap();
//! ```  
//! However, /pattern/ could be a part of two division expression sequence.
//! Therefore we will be looking into previous token to determine whether Regex literal is valid in the current position.
//!
//! ```
...
//! ### States
//!
//! Multiple state-based tokenizer may be required to parse many language syntaxes like Javascript template literal.
//! Thus, multiple states based [CombinedTokenizer]  lexical analyzer may be formed to tokenize syntax like Javascript template literal.
//! Each state provides a set of lexeme utilities that match relevant values or patterns for that particular state.  
//!
//! ```rust,no_run
...
//! ## Parser
//!
//! In this section, we will implement a Parser for Javascript expression.
//! We will use the tokenized data from the tokenizer and parse it into [ASTNode] according to the grammar of the language.
//! Once we received the tokens from the tokenizer we like to filter non-grammatical tokens like ‘Space’ from the token list to simplify and speed up parsing.
//! Therefore, we will update [is_structural](TokenImpl::is_structural) implementation to filter the non-structural token.
//!
//! ```
//! impl TokenImpl for Token {
//!     ...
...
//!
//! ```
//! We will also create Node values and implement [NodeImpl] trait to represent each node of the [AST](crate::ASTNode).
//!  
//! ```
...
//! ```
//!
//! Now, we will be implementing the grammar for parsing Javascript expressions.
//! Before writing our complete expression we will first implement a parser for simple arithmetic expressions.
//!
//! ```no_run
...
//!
//! ```
//! By default, the production utilities [Concat](crate::production::Concat),
//! [List](crate::production::List), or [SeparatedList](crate::production::SeparatedList) do not create any node in the parsed tree.
//! Instead, they flatten the parsed tree and append it into a Vec.
//! It is required to wrap a utility with [Node](crate::production::Node) to create a node in the [AST](ASTNode).
//! Let wrap multiplicative_term, addition, and expression with [Node](crate::production::Node).
//!
//! ```no_run
//! #[derive(Debug, Clone, Copy)]
//! enum NodeValue {
...
//!
//! Our current parser is not designed to parse higher-order expressions like truthy, instance-of expression, etc.
//! Let us update our parser to parse higher-order expressions.
//!
//! ```no_run
...
//!
//! Our current implementation require a semicolon(;) to terminate the javascript expression.
//! However, a Javascript expression can also be termination by eof, close brace (}) or by line break character.
//! We do not want to consume eof or close brace character because they are part of another production.
//! Therefore, we will be implementing [Lookahead](crate::production::Lookahead) utility to check if eof or '}'
//! exist immediately after an expression.
//!
//! Moreover, a new line character can also indicate a expression termination syntax for the Javascript language.
//! However, [is_structural](TokenImpl::is_structural) implementation filtered out LineBreak tokens from token stream.
//! Therefore, we will using [NonStructural](crate::production::NonStructural) utility to enforce child production to
//! consume unfiltered token stream. The complete production for expression termination is given below.
//!
//! ```no_run
...
//!
//! # License
//! [lang_pt](crate) is provided under the MIT license. See LICENSE.